...
# Create results container that will appear right after input
results_container = st.container()

# Section 1: IP Input - Compact Design
st.markdown("""
...
with tab1:
    ...
    # Prominent Demo IPs - Better mobile layout
    st.markdown("#### ✈️ Try These Aviation Companies:")
    
    # Mobile-friendly demo buttons - 2 columns instead of 6
    demo_ips = [
        {"name": "🛩️ Boeing", "ip": "52.16.0.0", "color": "#1f77b4"},
        {"name": "✈️ Delta Airlines", "ip": "199.168.0.0", "color": "#ff7f0e"},
        {"name": "🛫 American Airlines", "ip": "104.244.0.0", "color": "#2ca02c"},
        {"name": "🔧 Lufthansa Technik", "ip": "64.233.160.0", "color": "#d62728"},
        {"name": "🌍 United Airlines", "ip": "157.240.0.0", "color": "#9467bd"},
        {"name": "⚙️ Rolls-Royce", "ip": "208.67.222.0", "color": "#8c564b"}
    ]
    
    # Create 2 columns for mobile, 3 for desktop
    cols = st.columns(2)
    for i, demo in enumerate(demo_ips):
        with cols[i % 2]:
            if st.button(
                ...
                help=f"Click to analyze {demo['name']} visitor"
            ):
                single_ip = demo['ip']
                process_single = True
                # Immediately scroll to results
                st.rerun()
...
# Process single IP if requested and display results immediately
...
# Display results immediately after processing - Mobile-friendly
        ...
        with contacts_tab:
            ...
            with summary_tab:
                ...
                <div class="mobile-summary">
                    ...
                    <p style="margin: 0; color: #666; font-size: 0.9rem;">{employees} employees • {company_info['industry']}</p>
                </div>
                """, unsafe_allow_html=True)
